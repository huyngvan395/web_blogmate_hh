                ...
                <main class="flex flex-col gap-4 w-full md:w-7/12 md:pr-14 order-2 md:order-1">
                    <ul class="flex items-center gap-2">
                        ...
                        <li class="block py-3"><a href="{{route('user.info.follower',['infoUser'=> Str::before($user->email, '@') ])}}"
                                class="block box-content text-black text-base hover:text-black">Người theo dõi</a>
                        </li>
                    </ul>
                    <div class="flex items-center">
                        <h1 class="text-4xl font-bold">{{$user->followers->count()}} người theo dõi</h1>
                    </div>
                    {{-- followers-contain --}}
                    <div class="flex flex-col">
                        <div class="">
                            @foreach($user->followers as $follower)
                                {{-- user--}}
                                <div x-data="followData({{ Auth::user()->followers->contains($follower->id) ? 'true' : 'false' }}, {{ $follower->id }})" class="flex justify-between w-full py-3">
                                    <div class="flex justify-center items-center gap-2">
                                        <div class="flex justify-center items-center relative">
                                            <a href="{{route('user.info',['infoUser'=> Str::before($follower->email, '@') ])}}" class="block">
                                                <img src="{{asset('storage/'.$follower->avatar)}}"
                                                    class="w-10 h-10 hover:opacity-60 rounded-full" alt="">
                                            </a>
                                        </div>
                                        <div class="flex">
                                            <a href="{{route('user.info',['infoUser'=> Str::before($follower->email, '@') ])}}">
                                                <h1 class="hover:underline">{{$follower->name}}</h1>
                                            </a>
                                        </div>
                                    </div>
